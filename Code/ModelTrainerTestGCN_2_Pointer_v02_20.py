...
n_cpus = 32  # Number of environments
...
# Do not create per-run trainer telemetry file handlers. Use the application's logging configuration.
...
class AttentionPointerNetwork(nn.Module):
    """
    Replaces LSTM Pointer Network with Multi-Head Attention (Transformer-style).
    Treats edges as a SET (permutation invariant), not a SEQUENCE.
...
if __name__ == "__main__":
    ...
    model = MaskablePPO(
        ...
        batch_size=400,  # Reduced from 2400 to fix CUDA OOM (Attention layer is memory hungry)
        gamma=0.999,
        policy_kwargs=policy_kwargs
    )
    
    # Train with callbacks
    checkpoint_callback = CheckpointCallback(save_freq=3125, 
